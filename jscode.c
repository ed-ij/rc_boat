...
void read_joystick_axis(int fd, struct js_event js, int thrust, int js_bv, float js_d, int js_m, int rudder) {    // another function, including the integer fd
    ...
    while (read (fd, &js, sizeof(js)) > 0) {                    // while there is a joystick event to read
        if (js.type == JS_EVENT_AXIS && js.number == 1) {       // and if that event is an axis event on the right axis */
            ...
            rudder = -(js.value);
            rudder = ((rudder+js_bv)/js_d)*js_m;               // variables used to make this value out of 2000
            printf ("1%d\n", rudder);
        }
    }
}

void js_maths(int js_bv, float js_d, int js_m) {            // giving the variables for the maths their values
    js_bv = 32767;
    js_d = 65.534;
    js_m = 2;
}
...
int main() {
    ...
    float js_d;
    int js_m;
    js_maths(js_bv, js_d, js_m);
    fd = open_joystick(fd);
    while (1) {
        read_joystick_axis(fd, js, thrust, js_bv, js_d, js_m, rudder);
        fflush(stdout);
    }
    return 0;
}
